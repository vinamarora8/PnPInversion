...
class PNP(nn.Module):
    def __init__(self, model_key, n_timesteps=NUM_DDIM_STEPS, device="cuda"):
        ...
        self.text_encoder = T5EncoderModel.from_pretrained(t5_name).to(device)
        self.text_encoder.eval()

        self.model_max_length = pipe.text_encoder.config.max_position_embeddings

        self.adapter = TextEncoderAdapter(
            input_dim=self.text_encoder.config.d_model,
            output_dim=pipe.text_encoder.config.hidden_size,
        ).to(device)

        ckpt = torch.load("adapt_t5/outputs/laion_sd1p5_t5_v1_1_xxl_mlp/adapter.pth")
    ...
    @torch.no_grad()
    def get_single_text_embed(self, prompt):
        ...
        adapter_embeddings = self.adapter(text_embeddings, text_input.attention_mask)
        return adapter_embeddings

    @torch.no_grad()
    def get_text_embeds(self, prompt, negative_prompt, batch_size=1):
        # Tokenize text and get embeddings
        text_embeddings = self.get_single_text_embed(prompt)
        uncond_embeddings = self.get_single_text_embed(negative_prompt)
