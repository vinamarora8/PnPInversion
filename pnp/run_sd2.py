...
pipe = DiffusionPipeline.from_pretrained(model_key, torch_dtype=torch.float16, variant="fp16")
toy_scheduler = DDIMScheduler.from_config(
...
timesteps_to_save, num_inference_steps = get_timesteps(
    toy_scheduler, num_inference_steps=NUM_DDIM_STEPS, strength=1.0, device=device
)
...
def edit_image_ddim_PnP(
    image_path, prompt_src, prompt_tar, guidance_scale=7.5, image_shape=[512, 512]
):
    # torch.cuda.empty_cache()
    image_gt = load_512(image_path)
    _, rgb_reconstruction, latent_reconstruction = model.extract_latents(
        data_path=image_path, num_steps=NUM_DDIM_STEPS, inversion_prompt=prompt_src
    )

    edited_image = pnp.run_pnp(
    ...
    return Image.fromarray(
        np.concatenate(
            (
                ...
                np.uint8(
                    255
                    * np.array(rgb_reconstruction[0].permute(1, 2, 0).cpu().detach())
                ),
                np.uint8(
...
def edit_image_directinversion_PnP(
    ...
    # torch.cuda.empty_cache()
    image_gt = load_512(image_path)
    inverted_x, rgb_reconstruction, _ = model.extract_latents(
        data_path=image_path, num_steps=NUM_DDIM_STEPS, inversion_prompt=prompt_src
    )
        ...
        np.concatenate(
            (
                ...
                np.uint8(
                    np.array(
                        latent2image(
                            model=pnp.vae, latents=inverted_x[1].to(pnp.vae.dtype)
                        )[0]
                    )
                ),
                np.uint8(
...
def parse_args():
    ...
    # parser.add_argument(
    #     "--edit_method_list", nargs="+", type=str, default=["ddim+pnp"]
